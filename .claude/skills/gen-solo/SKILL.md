...
2. `data-gen/INTERVIEW.md` — 対話式ヒアリングの質問バンク（Step 1 で使う）。
3. `data-gen/templates/header.alphatex` — 多トラックのヘッダ雛形。
4. `data-gen/templates/{grooves,fills,rudiments}.md` — 固定の基礎素材語彙。
5. `data-gen/phrase-library/README.md` — 育てる分類済みフレーズ集（想起/記録の規約）。
6. `data-gen/BRIEF.md` — 依頼フォーマット。
...
- Step 1.5 の想起フレーズと `templates/*.md` の素材を sections に沿って並べ、必要に応じて変奏する。
